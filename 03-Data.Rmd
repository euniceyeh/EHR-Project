...
zip_dat %>% filter(nchar(zip) < 5) %>% select(npi, frst_nm, zip, cty, st, city, state) %>% head(15)
```
...
[do i need to explain the analysis-ready data?]
...
hosp_afl <- clean %>% # filter on the clean version
  filter(!is.na(as.integer(hosp_afl_1))) %>% 
  mutate(hosp_afl_lbn_1 = as.character(hosp_afl_lbn_1), st = as.character(st)) %>% 
  # only keep relevant or potentially interesting variables
  select(hosp_afl_lbn_1, st, cty, zip, ehr, hosp_afl_1, pri_spec, gndr, grd_yr)
...
```{r}
hosps <- inner_join(hosp_afl, hosp_clean, by=c("hosp_afl_lbn_1"="Hospital_name", "st"="State"))
summary(hosps)
```

We managed to get a `{r} (nrow(hosps)/nrow(hosp_clean))*100`% match on the hospitals between the two data sets! And the summary statistics on those hospital demographics look a lot nicer and less skewed! _**<------actually check this again on final run!!!!**_

### Output
```{r}
# clean up intermediate data sets
rm(hosp_web, hosp_clean, hosp_afl)
...
* It sounds like the first step we may need to do is to split the data into EPs and hospitals since there are enough distinct variables relevant for each type. Then we can merge each split with their corresponding demographic data from the previous sections.

  + We want to left join with the `EPs` data set on `NPI`, which has one row per physician by their unique NPI (one-to-many merge)
  + We want to left join with the `hosps` data set on `CCN`, which has one row per hospital by their unique CCN (one-to-many merge)
  
* Then separately check on their summaries again?
...
# merge with EPs data
EHR_EPs <- left_join(EHR_EPs, EPs, by=c('NPI'='npi'))

# split for hospitals
EHR_hosps <- EHR_clean %>% filter(Provider_Type == 'Hospital') %>% 
  select(-ZIP, -Specialty, -Provider_Type)
# merge with hosps data
EHR_hosps <- left_join(EHR_hosps, hosps, by=c('CCN'='hosp_afl_1'))
```


why are there so many duplicate rows of the same thing?
...
Later: 
  ...
  - notice that the `ehr` column is not all `y` when we expect them to be. this explains why we had so many physicians who fell in the `ehr=''` category. the physicians whom we have EHR product usage information on did not answer "yes" to EHR use in the original `EPs` data.
  - again the top primary specialties overall are `r summary(EPs$pri_spec, maxsum=6)[1:5]`, compare that to the top specialities among physicians whom we have vendor information: `r summary(EHR_EPs$pri_spec, maxsum=6)[1:5]`.
